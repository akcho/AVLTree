...
void AVL<T>::printLevelOrder() const {
	queue<AVLNode<T>*> q;	// we will fill use q to store random integers from main()
	...
	while (!q.empty()) {	// this shouldn't be empty, since we just added the first node
		...
		if (front->right)	// if root's ride node isn't null...
...
void AVL<T>::doRotation(AVLNode<T>* point) {
	...
	if (leftChild > rightChild) {	// CR, but is it single or double?
		...
		int leftGC = -1;
		int rightGC = -1;
		if (point->right->left != nullptr)	// if the right node's left child is null...
			leftGC = point->right->left->height;	// the 
